...
```{r}
proteinCrop <- image_crop(protein,geom)
cilierCrop <- image_crop(cilier,geom)
pltCilierCrop <- image_ggplot(cilierCrop)
pltProteinCrop <- image_ggplot(proteinCrop)
# Spatial correlation is not very clear, since some protein seem to correlate with the shape of the cilia, but other protein does not.
library(gridExtra)
grid.arrange(pltCilierCrop,pltProteinCrop,nrow=1)
```
...
proteinCropBM <- proteinCrop[[1]]
dim(proteinCropBM)
dim(cilierCropBM)
cilierCropBM[,20,15]
proteinCropBM[,20,15]
# We expected a high number in the green channel of CilierCropBM and in the red channel of proteinCropBM, which was confirmed.
```
...
```{r}
proteinCropR <- image_channel(proteinCrop,channel="red")
proteinCropG <- image_channel(proteinCrop,channel="green")
proteinCropB <- image_channel(proteinCrop,channel="blue")
pltRed<-qplot(as.double(proteinCropR[[1]]),geom="histogram",bins=100)
pltRed<-pltRed+labs(title="Histogram of proteinCropR",x="intensity",y="count")
pltGreen<-qplot(as.double(proteinCropG[[1]]),geom="histogram",bins=100)
pltGreen<-pltGreen+labs(title="Histogram of proteinCropG",x="intensity",y="count")
pltBlue<-qplot(as.double(proteinCropB[[1]]),geom="histogram",bins=100)
pltBlue<-pltBlue+labs(title="Histogram of proteinCropB",x="intensity",y="count")
grid.arrange(pltRed,pltGreen,pltBlue,nrow=1,ncol=3)
# The cilier histograms show predominantly activity in the green channel, while the protein histograms show mostly activity in the red channel. We interpret the protein histogram for the blue channel to mean that only one bin has been calculated corresponding to all values being zero in that channel.
```
...
cilierHSL <-image_convert(cilierCrop, colorspace="HSL")
cilierCropL <- image_channel(cilierHSL,channel="Lightness")
cilierCropLEq <- image_equalize(cilierCropL)

pltCilierCropGray <- image_ggplot(cilierCropGray)
pltCilierCropGrayEq <- image_ggplot(cilierCropGrayEq)
pltCilierCropGrayHist <- qplot(as.double(cilierCropGray[[1]]),geom="histogram")+labs(title="cilierCropGray",x="intensity",y="count")

pltCilierCropL <- image_ggplot(cilierCropL)
pltCilierCropLEq <- image_ggplot(cilierCropLEq)
pltCilierCropLHist <- qplot(as.double(cilierCropL[[1]]),geom="histogram")+labs(title="cilierCropL",x="intensity",y="count")

grid.arrange(pltCilierCropGray,pltCilierCropGrayEq,pltCilierCropGrayHist,
             pltCilierCropL,pltCilierCropLEq,pltCilierCropLHist,
             nrow=2,ncol=3)
# The Lightness channel is similar to the gray image but generally seems darker. Histogram equalization makes them more similar.
```
...
cilierCropGray <- image_convert(cilierCrop,colorspace = "GRAY")
cilierCropGrayEq <- image_equalize(cilierCropGray)
cilierCropAvg <- image_fx(cilierCrop,"(u.r+u.g+u.b)/3.0")
cilierCropAvgEq <- image_equalize(cilierCropAvg)

pltCilierCropGray <- image_ggplot(cilierCropGray)
pltCilierCropGrayEq <- image_ggplot(cilierCropGrayEq)
pltCilierCropGrayHist <- qplot(as.double(cilierCropGray[[1]]),geom="histogram")+labs(title="cilierCropGray",x="intensity",y="count")

pltCilierCropAvg <- image_ggplot(cilierCropAvg)
pltCilierCropAvgEq <- image_ggplot(cilierCropAvgEq)
pltCilierCropAvgHist <- qplot(as.double(cilierCropAvg[[1]]),geom="histogram")+labs(title="cilierCropAvg",x="intensity",y="count")

grid.arrange(pltCilierCropGray,pltCilierCropGrayEq,pltCilierCropGrayHist,
             pltCilierCropAvg,pltCilierCropAvgEq,pltCilierCropAvgHist,
             nrow=2,ncol=3)
# These two images are rather similar, however, our average image is slightly darker, and the histogram equalized images also differs a little in the low-contrast regions.
```
...
cilierCropGrayGradScaled<-image_fx(cilierCropGrayGrad,"(u-minima)/(maxima-minima)")
image_ggplot(cilierCropGrayGradScaled)
```

---

## Assignment 1.7
Using image_fx, make an image cilierCropGrayScaled, where intensities are streched to fill the entire range [0,1] using f(u)=(u-minima)/(maxima-minima). Compare this to a new image cilierCropGrayLn where all intensities have been transformed by the ln-function as f(p) = ln(1+5*p), and finally stretched to fill the entire range. Visualize the stretched, their histograms in a 2x2 grid. What is the qualitative effect of the ln-transformation?

```{r}
cilierCropGrayScaled<-image_fx(cilierCropGray,"(u-minima)/(maxima-minima)")
cilierCropLn <- image_fx(cilierCropGray,"ln(1+5*u)")
cilierCropLnScaled<-image_fx(cilierCropLn,"(u-minima)/(maxima-minima)")
pltOrig<-image_ggplot(cilierCropGrayScaled)
pltOrigHist <- qplot(as.double(cilierCropGrayScaled[[1]]),geom="histogram")+labs(title="cilierCropGrayEq",x="intensity",y="count")
pltIm<-image_ggplot(cilierCropLnScaled)
pltHist <- qplot(as.double(cilierCropLnScaled[[1]]),geom="histogram")+labs(title="cilierCropGrayEq",x="intensity",y="count")
grid.arrange(pltOrig,pltOrigHist,pltIm,pltHist,nrow=2,ncol=2)
# The logarithm seems to lift the light areas slightly more than the dark areas. There seems to be an overflow/saturation in some middle part of the cilier, where a couple of pixels all have turned into the maximum possible.
```
...
```{r}
seq <- image_join(cilierCrop,proteinCrop)
comp <- image_fx_sequence(seq,"(u+v)/2")
compStretched<-image_fx(comp,"(u-minima)/(maxima-minima)")
image_ggplot(compStretched)
# The average color is mainly green or red indicating that there is only a weak correlation between the cilier and the protein.
```
