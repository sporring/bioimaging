...
>How many red pixels there are in the vicinity of the bright green pixels?
...
The threshold 0.1 was arbitrarily chosen, so let's investigate several thresholds:
...
## Assignment 2.1
The image proteinGray is very dark, so stretched it to full range [0,1] using image_fx, and call the result proteinScaled. Find a suitable threshold-value proteinScaled as proteinT, and calculate its area as proteinArea.
...
## Assignment 2.2
Sum the amount of intensities in proteinGray in the pixels of the thresholded cilier image plus a neighborhood of 3 pixels away from the cilier. 
...
## Assignment 2.3
Find a threshold for proteinGray, which has a black whole, and use a single dilation followed by a single erosion with the same structure element to remove the whole. Subtract the thresholded image form the dilated-erode image and visualize the result. Did the process perform as expected?
